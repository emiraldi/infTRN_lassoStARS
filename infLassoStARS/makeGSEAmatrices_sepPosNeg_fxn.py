...
INPUTS:
	...
	fdrCut -- geneset AND TF must have one enrichment at this FDR cutoff in order to be 
	...
	fdrMax -- if non-zero, then a sparse network output will be made (3 column format:
		TFs, gene, interaction weight = -log10(max(adjp,fdrMax)) * sign (up or down) )
OUTPUTS:
	Tables (gene sets X TFs, where TFs are separated by + and - edges, if needed)
	outDir1_${gseaSetsName}_fdr${fdrCut}_pvals_${sign} -- raw pvalues [sets X TFs]
	outDir1_${gseaSetsName}_fdr${fdrCut}_adjps_${sign} -- adj. pvalues [sets X TFs]
	outDir1_${gseaSetsName}_fdr${fdrCut}_numGenes_${sign} -- # genes in overlap [sets X TFs]
	outDir1_${gseaSetsName}_fdr${fdrCut}_genes_${sign} -- space-separated genes in overlap [sets X TFs]
"""	
...
# keep track of sets that reached fdrCut in a list; do the same for TFs
enrichDic = dict()
setDic = dict()
...
for direction in directionSymbolReplacements.keys():
	outDir1 = outFolderBase + '/' + gseaSetsName + '_fdr' + str(int(100*fdrCut)) + '_' + direction
