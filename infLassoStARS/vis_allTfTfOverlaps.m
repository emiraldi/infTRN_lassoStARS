function vis_allTfTfOverlaps(datasetName, tfPairMat, desClusts, titleInf,... 
    fullFigOutDir, aveGeneExprMat, annotations, saveFig, axisFontSize,...
    figureDimensions)
%% vis_allTfTfOverlaps(datasetName, tfPairMat, desClusts, titleInf,... 
%    fullFigOutDir, aveGeneExprMat, annotations, saveFig, axisFontSize,...
%    figureDimensions)
%% GOAL: cluster/ visualize TF-TF modules and normalized overlaps from 
% calc_zscoredTfTfOverlaps.m. This function produces several plots:
% 1. dendrogram -- subsequent plots order TFs according to this dendrogram
...
% desClusts -- desired number of clusters; NOTE: clustering is 
%   hierarchical, so affects cluster visualization only)
...
% % required inputs:
% datasetName = 'Th17';
% tfPairMat = 'outputs/networks_targ0p05_SS50_bS5/Network0p05_15tfsPerGene/ATAC_Th17_bias50_maxComb/ATAC_Th17_bias50_maxComb_cut01/zOverlaps_combEdge_fdr100_tfMin20_targMin1/tfPair.mat';
% desClusts = 10;     % desired number of clusters (clustering is hierarchical, so affects visualization only)
...
% 
% fullFigOutDir = fullfile(outDir,['fullFigs_clust' num2str(desClusts)]);
% %% OPTIONAL INPUTS:
...
% axisFontSize = 2;   % heatmap fontsize
% figureDimensions = [7 12]; % 2D vector of (x-size, y-size) dimensions for
% %   saving pdfs of the figures, units are inches
...
figure(3), clf
subplot(1,4,3:4)
barh(flipud(silScores),'FaceColor','k')
axis tight
set(gca,'YTick',1:totTfs,'YTickLabel',strvcat(sigTfs{fliplr(horderFull)}),...
...
%     for ss = 1:desClusts        
%         plot(ax(1:2),((totTfs-start_spotsS(ss))*[1 1]-.5),'Color',.25*ones(1,3))
%     end    
if saveFig
...
%% 4. similarity matrix normalized overlaps
plotMat = tfPairAnal.zMat(horderFull,horderFull)+max(tfPairAnal.zMat(:))*eye(totTfs);
currLabels = cellstr(strvcat(sigTfs{horderFull}));
figure(4), clf
imagesc(plotMat)
if min(plotMat(:)) < 0
...
ax = axis();
for ss = 1:desClusts
    plot(ax(1:2),(start_spots(ss)*[1 1]-.5),'k','LineWidth',lineWidth)
    plot([start_spots(ss)*[1 1]-.5], ax(3:4),'k','LineWidth',lineWidth)
end
title([titleInf ':: Normalized Overlap'],'FontSize',titleFontSize)        
set(gca,'YTick',1:totTfs,'YTickLabel',strvcat(currLabels),...
    'FontSize',axisFontSize)%,'FontWeight','Bold')
set(gca,'XTick',1:totTfs,'XTickLabel',strvcat(currLabels),...
...
%% 5. similarity matrix based on raw overlaps
plotMat = tfPairAnal.sigPerOverlaps(horderFull,horderFull)+max(tfPairAnal.sigPerOverlaps(:))*eye(totTfs);
currLabels = cellstr(strvcat(sigTfs{horderFull}));
figure(5), clf
imagesc(plotMat)
colormap hot
...
ax = axis();
for ss = 1:desClusts
    plot(ax(1:2),(start_spots(ss)*[1 1]-.5),'r','LineWidth',lineWidth)
    plot([start_spots(ss)*[1 1]-.5], ax(3:4),'r','LineWidth',lineWidth)
end
...
%% 6. similarity matrix based on raw p-val Fisher exact test
plotMat = (-tfPairAnal.sigRawSigs(horderFull,horderFull))+max(-tfPairAnal.sigRawSigs(:))*eye(totTfs);
currLabels = cellstr(strvcat(sigTfs{horderFull}));
figure(6), clf
imagesc(plotMat)
...
%% 7. similarity matrix based on adjusted p-val Fisher exact test
plotMat = (tfPairAnal.sigSigs(horderFull,horderFull)+max(tfPairAnal.sigSigs(:))*eye(totTfs));
currLabels = cellstr(strvcat(sigTfs{horderFull}));
figure(7), clf
imagesc(plotMat)
colormap(flipud(gray))
...
if length(aveGeneExprMat) > 0
    ...
    geneExprOrd = zeros(totTfs,totConds);
    [names,porder,corder] = intersect(upper(genesc),sigTfsOrdered);
    ...
    imagesc(transVals')
    colormap(redblue)
    set(gca,'XTick',1:totTfs,'XTickLabel',sigTfsOrdered,...
...
for gs = 1:totEnrich
    ...
    % reorder TFs
    padjsOrd = ones(totTfs,totSets);
    [names,porder,corder] = intersect(tfsTmpc,sigTfsOrdered);
    ...
    imagesc(transVals)
    colormap redblue
    set(gca,'YTick',1:totTfs,'YTickLabel',strrep((sigTfsOrdered),'_', ' '),...
